...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, CommonModule, PageBackButtonComponent],
...
export class CheckoutComponent implements OnInit {
  ...
  if (!userId) {
    this.toast.show('Invalid login session. Please login again', 'error');
    this.router.navigate(['/login']);
    return;
  }
  ...
  const fullAddress = `${this.form.value.address}, ${this.form.value.city}, ${this.form.value.pincode}`;
  ...
  const orderDTO: OrderDTO = {
    ...
    paymentMethod: this.form.value.paymentMethod || 'cod',
    items
  };
